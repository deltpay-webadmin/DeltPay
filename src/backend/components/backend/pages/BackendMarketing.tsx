...
//
// Live mode only — real Meta campaign/day insights (marketingStore) blended
// with CRM stages: Meta supplies impressions → clicks → leads; the CRM
// supplies qualified → converted for Meta-sourced leads. Until an ad account
// connects, the page shows a connect empty state.
// ══════════════════════════════════════
...
export function BackendMarketing() {
  ...
  // No connected ad account (or no synced insight rows yet) — show a connect
  // empty state instead of the funnel/stage/channel/trend sections.
  if (!live) {
    return (
        ...
        <div className="max-w-[1360px] mx-auto px-4 lg:px-8 py-6 space-y-6">
          <Card title="Marketing performance">
            <div className="py-12 text-center">
              <Link2 className="w-8 h-8 text-(--dp-text-faint) mx-auto mb-3" />
              <p className="text-[14px] font-semibold text-(--dp-text)">No ad data yet</p>
              <p className="mt-1 text-[12px] text-(--dp-text-muted)">
                Connect Meta Ads to see live funnel data — spend, impressions, leads, and CRM conversions.
              </p>
              <div className="mt-5 flex justify-center">
                <Btn variant="primary" size="sm" onClick={() => setConnectOpen(true)}>
                  <Link2 className="w-3.5 h-3.5" /> {metaConn ? 'Manage connection' : 'Connect Meta Ads'}
                </Btn>
              </div>
            </div>
          </Card>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_460px] gap-8 p-6 lg:p-8">
            <div className="flex flex-col justify-center">
              <Overline className="text-white/60">
                {`Meta Ads · last 30 days · ${live.period}`}
              </Overline>
