...
// Analytics inputs — populated from settled disputes and processor reporting.
interface EvidenceCombo { combo: string; used: number; wins: number; losses: number }
const EVIDENCE_COMBOS: EvidenceCombo[] = [];

interface MerchantCbRatio { merchant: string; txnCount: number; cbCount: number; network: string; threshold: number }
const MERCHANT_CB_RATIOS: MerchantCbRatio[] = [];

const CB_RATIO_MONITOR: (MerchantCbRatio & { ratio: number })[] = [];
...
export function BackendDisputes() {
  ...
  const workflowCount = DISPUTES.filter(d => !['won', 'lost'].includes(d.stage)).length;
  const pendingAlertCount = PRE_ALERTS.filter(a => a.status === 'pending').length;

  const tabs = [
    { key: 'inbox' as const, label: 'Dispute Inbox', badge: activeDisputes.length || undefined },
    { key: 'workflow' as const, label: 'Representment Workflow', badge: workflowCount || undefined },
    { key: 'codes' as const, label: 'Reason Codes' },
    { key: 'analytics' as const, label: 'Win/Loss Analytics' },
    { key: 'alerts' as const, label: 'Pre-CB Alerts', badge: pendingAlertCount || undefined },
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    ...
                    {activeDisputes.length === 0 && (
                      <tr>
                        <td colSpan={7} className="px-5 py-12 text-center text-sm text-gray-400">No active disputes — response deadlines will appear here.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-[8px] border border-gray-200">
                ...
                <div className="px-5 py-4 space-y-3">
                  ...
                  {Object.keys(analytics.byVertical).length === 0 && (
                    <p className="py-8 text-center text-xs text-gray-400">No vertical data yet.</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-[8px] border border-gray-200">
                ...
                <div className="px-5 py-4 space-y-3">
                  ...
                  {Object.keys(analytics.byEvidence).length === 0 && (
                    <p className="py-8 text-center text-xs text-gray-400">No evidence data yet.</p>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    ...
                    {EVIDENCE_COMBOS.length === 0 && (
                      <tr>
                        <td colSpan={6} className="px-5 py-12 text-center text-sm text-gray-400">No evidence combinations analysed yet — results appear once disputes are settled.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    ...
                    {PRE_ALERTS.length === 0 && (
                      <tr>
                        <td colSpan={7} className="px-5 py-12 text-center text-sm text-gray-400">No pre-chargeback alerts yet — Verifi and Ethoca notifications will appear here.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="bg-white rounded-[8px] border border-gray-200">
              ...
              <div className="px-5 py-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  {CB_RATIO_MONITOR.length === 0 && (
                    <p className="py-8 text-center text-xs text-gray-400">No chargeback ratio data yet — merchant ratios will appear here once processing reporting is connected.</p>
                  )}
                </div>
              </div>
            </div>
                  ...
                  <table className="w-full min-w-[1000px]">
                    ...
                    <tbody>
                      ...
                      {disputeCosts.length === 0 && (
                        <tr>
                          <td colSpan={10} className="px-5 py-12 text-center text-sm text-gray-400">No disputes to cost out yet.</td>
                        </tr>
                      )}
                      {/* Totals row */}
                  ...
                  <div className="px-5 py-4 space-y-3">
                    ...
                    <div className="bg-brand/5 rounded-[6px] border border-brand/20 p-3 mt-2">
                      ...
                      <p className="text-xs text-gray-600 leading-relaxed">
                        ...
                        Not fighting disputes costs <strong className="text-red-700">{fmt(totalInaction)}</strong> across the portfolio — {totalRepresentmentCost > 0 ? `${(totalInaction / totalRepresentmentCost).toFixed(1)}×` : '—'} more than the cost of fighting them.
                      </p>
                    </div>
                  </div>
                </div>
