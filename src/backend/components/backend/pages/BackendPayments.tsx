...
export function BackendPayments() {
  ...
  const successfulCollections = PAYMENTS.filter(p => p.date === today && p.status === 'success' && p.type === 'collection');
  const todayCollected = successfulCollections.reduce((s, p) => s + p.amount, 0);
  ...
  const activeCollections = COLLECTIONS.filter(c => c.status !== 'paid_off');
  const avgCollectionRate = activeCollections.length
    ? activeCollections.reduce((s, c) => s + c.percentPaid, 0) / activeCollections.length
    : 0;
  ...
  return (
    <div className="px-6 py-6 space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: "Today's Collections", value: `$${todayCollected.toLocaleString()}`, sub: `${successfulCollections.length} successful`, color: 'border-t-emerald-500', icon: DollarSign },
          { label: 'Failed Today', value: todayFailed, sub: `${todayFailed} NSF returns`, color: 'border-t-red-500', icon: XCircle },
          { label: 'Outstanding Balance', value: `$${totalOutstanding.toLocaleString()}`, sub: `${activeCollections.length} active accounts`, color: 'border-t-brand', icon: Banknote },
        ...
        <>
          ...
          <div className="bg-white rounded-[8px] border border-gray-200 overflow-hidden">
            ...
            {filteredPayments.map(pmt => {
              ...
              return (
                <div key={pmt.id} className={`px-4 py-3 flex items-center gap-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50/50 transition-colors ${pmt.status === 'failed' ? 'bg-red-50/20' : ''}`}>
                  ...
                  <span className="w-20 shrink-0 text-[10px] font-mono text-gray-500">{pmt.date}</span>
            ...
            {filteredPayments.length === 0 && (
              <div className="px-5 py-12 text-center text-sm text-gray-400">No payments yet</div>
            )}
          </div>
        </>
      ) : (
        <>
          ...
          <div className="bg-white rounded-[8px] border border-gray-200 overflow-hidden">
            ...
            {filteredCollections.length === 0 && (
              <div className="px-5 py-12 text-center text-sm text-gray-400">No collection accounts yet</div>
            )}
          </div>
        </>
      )}
