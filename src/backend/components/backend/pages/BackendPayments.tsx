    ...
    <div className="px-6 py-6 space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: 'Avg % Collected', value: COLLECTIONS.length > 0 ? `${avgCollectionRate.toFixed(1)}%` : '—', sub: 'across active deals', color: 'border-t-blue-500', icon: TrendingUp },
        ].map((kpi, i) => {
          const Icon = kpi.icon;
          return (
          ...
          <div className="bg-white rounded-[8px] border border-gray-200 overflow-hidden">
            ...
            {filteredPayments.length === 0 && (
              <div className="px-5 py-16 text-center">
                ...
                <p className="text-sm text-gray-400">{PAYMENTS.length === 0 ? 'No payments yet — merchant ACH collections, fundings, and fees will appear here once processing starts.' : 'No transactions match your filters'}</p>
              </div>
            )}
          </div>
        </>
          ...
          <div className="bg-white rounded-[8px] border border-gray-200 overflow-hidden">
            ...
            {filteredCollections.length === 0 && (
              <div className="px-5 py-16 text-center">
                ...
                <p className="text-sm text-gray-400">{COLLECTIONS.length === 0 ? 'No collection accounts yet — funded deals will appear here with their repayment progress.' : 'No accounts match your filters'}</p>
              </div>
            )}
          </div>
        </>
