...
export function BackendWebsites() {
  ...
  const convertedSites = WEBSITES.filter(w => w.conversionRate > 0);
  const avgConversion = convertedSites.length
    ? convertedSites.reduce((s, w) => s + w.conversionRate, 0) / convertedSites.length
    : 0;
    ...
    <div className="px-6 py-6 space-y-5">
      ...
      {filtered.length === 0 && (
        <div className="bg-white rounded-[8px] border border-gray-200 px-5 py-16 text-center">
          ...
          <p className="text-sm text-gray-400">No websites yet</p>
        </div>
      )}
