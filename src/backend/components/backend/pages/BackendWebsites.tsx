...
export function BackendWebsites() {
  ...
  const sitesWithConversion = WEBSITES.filter(w => w.conversionRate > 0);
  const avgConversion = sitesWithConversion.reduce((s, w, _, a) => s + w.conversionRate / a.length, 0);
    ...
    <div className="px-6 py-6 space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: 'Avg Conversion', value: sitesWithConversion.length > 0 ? `${avgConversion.toFixed(1)}%` : '—', sub: 'across live sites', color: 'border-t-amber-500', icon: TrendingUp },
        ].map((kpi, i) => {
          const Icon = kpi.icon;
          return (
      ...
      {filtered.length === 0 && (
        <div className="bg-white rounded-[8px] border border-gray-200 px-5 py-16 text-center">
          ...
          <p className="text-sm text-gray-400">
            {WEBSITES.length === 0
              ? 'No merchant websites yet — sites you build for merchants will appear here'
