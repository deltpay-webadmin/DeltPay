...
const upcomingDeadlines: { date: string; label: string; type: string }[] = [];

const agentCommissionPayouts: { name: string; deals: number; amount: number }[] = [];
    ...
    <div className="h-full flex flex-col bg-canvas">
      ...
      <div className="px-6 py-6 flex-1 overflow-y-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard label="Last Payroll Paid" value={fmt(lastPayroll)} icon={<CheckCircle className="w-5 h-5" />} sub="No runs yet" />
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 px-4 py-3 bg-brand/5 border border-brand/15 rounded-[8px]">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Semi-monthly cycle</p>
              </div>
            </div>
                    ...
                    {employeeItems.length === 0 && (
                      <tr>
                        <td colSpan={7} className="px-5 py-12 text-center text-sm text-gray-400">
                          No employee payees yet — they will appear here once employees are added.
                        </td>
                      </tr>
                    )}
                  </tbody>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    ...
                    {agentItems.length === 0 && (
                      <tr>
                        <td colSpan={7} className="px-5 py-12 text-center text-sm text-gray-400">
                          No agent commissions yet — payouts will appear here once deals are funded.
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        {tab === 'calendar' && (
          <div className="space-y-6">
              ...
              <div className="p-4">
                <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-[8px] overflow-hidden">
                  ...
                  {Array.from({length: 30}, (_, i) => i + 1).map(day => {
                    ...
                    return (
                      <div key={day} className="bg-white p-2 min-h-[72px]">
                        <span className="text-xs font-medium text-gray-700">{day}</span>
                        {isPayDay && (
                          <div className="mt-1 px-1.5 py-0.5 rounded text-[9px] font-semibold bg-brand/10 text-brand">
                            Pay Day
                          </div>
                        )}
                        {isDeadline && (
              ...
              <div className="space-y-2">
                ...
                {upcomingDeadlines.length === 0 && (
                  <p className="py-8 text-center text-sm text-gray-400">
                    No upcoming deadlines — payroll and filing dates will appear here.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-[8px] border border-gray-200 p-5">
              ...
              <p className="text-xs text-gray-500 mb-4">Commission data auto-synced from the Agent Commissions module.</p>
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-purple-50 rounded-[8px] p-3 text-center">
                  <p className="text-xl font-bold text-purple-700">{fmt(agentCommissionPayouts.reduce((s, a) => s + a.amount, 0))}</p>
                  <p className="text-[10px] text-purple-600 mt-0.5">Total Commissions This Period</p>
                </div>
                <div className="bg-purple-50 rounded-[8px] p-3 text-center">
                  <p className="text-xl font-bold text-purple-700">{agentCommissionPayouts.length}</p>
                  <p className="text-[10px] text-purple-600 mt-0.5">Agents With Payouts</p>
                </div>
                <div className="bg-purple-50 rounded-[8px] p-3 text-center">
                  <p className="text-xl font-bold text-purple-700">{agentCommissionPayouts.reduce((s, a) => s + a.deals, 0)}</p>
                  <p className="text-[10px] text-purple-600 mt-0.5">Deals Earning Commissions</p>
                </div>
              </div>
              <div className="mt-4 border-t border-gray-200 pt-3">
                <div className="space-y-2">
                  {agentCommissionPayouts.map(agent => (
                  ...
                  {agentCommissionPayouts.length === 0 && (
                    <p className="py-6 text-center text-sm text-gray-400">
                      No agent payouts this period.
                    </p>
                  )}
                </div>
              </div>
                  ...
                  {pastRuns.length === 0 && (
                    <tr>
                      <td colSpan={8} className="px-5 py-12 text-center text-sm text-gray-400">
                        No payroll runs yet — completed runs will appear here.
                      </td>
                    </tr>
                  )}
                </tbody>
