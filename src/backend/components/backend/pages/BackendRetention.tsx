    ...
    <div className="px-6 py-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-[8px] border border-indigo-200 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">Outreach engagement and merchant health</p>
            <p className="text-xs text-gray-500 mt-0.5">Engagement trends will appear here once outreach campaigns are running.</p>
          </div>
        </div>
              ...
              {merchants.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-4 py-12 text-center text-sm text-gray-400">
                    No merchants monitored yet — health scores will appear here once merchants are onboarded.
                  </td>
                </tr>
              )}
            </tbody>
...
function DetailPanel({ merchant, tasks, onAddTask, onClose }: {
  ...
  const trendFirst = merchant.volumeTrend[0] ?? 0;
  const trendLast = merchant.volumeTrend[merchant.volumeTrend.length - 1] ?? 0;
  const trendDecline = trendFirst > 0
    ? Math.round(((trendFirst - trendLast) / trendFirst) * 100)
    : 0;
