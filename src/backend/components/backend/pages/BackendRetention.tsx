...
// Data will be supplied by the retention service.
...
function Sparkline({ data, color = '#2E6BFF' }: { data: number[]; color?: string }) {
  if (data.length < 2) {
    return <div className="h-9 flex items-center text-xs text-gray-400">No trend data yet</div>;
  }
    ...
    <div className="px-6 py-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-[8px] border border-indigo-200 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">Outreach engagement insights will appear here</p>
            <p className="text-xs text-gray-500 mt-0.5">No outreach engagement data is available yet.</p>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {merchants.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-5 py-12 text-center text-sm text-gray-400">No at-risk merchants yet</td>
                </tr>
              )}
            </tbody>
          </table>
...
function DetailPanel({ merchant, tasks, onAddTask, onClose }: {
  ...
  const firstVolume = merchant.volumeTrend[0] ?? 0;
  const lastVolume = merchant.volumeTrend.at(-1) ?? 0;
  const trendDecline = firstVolume > 0
    ? Math.round(((firstVolume - lastVolume) / firstVolume) * 100)
    : 0;
